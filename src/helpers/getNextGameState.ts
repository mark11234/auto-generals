...
export const getNextGameState = (gameState: GameState): GameState => {
  ...
  if (nextGameState.turn % 25 === 0) {
    return {
      ...nextGameState,
      rows: nextGameState.rows.map((row) =>
        row.map(
      ...
      rows: nextGameState.rows.map((row) =>
        row.map(
          (cell: CellState): CellState => ({
            ...cell,
            value:
              (cell.type === "Castle" || cell.type === "Crown") && cell.owner
                ? cell.value + 1
                : cell.value,
          })
        )
...
const doMove = (
  ...
  if (nextGameState.rows[move.to.row][move.to.column].owner === bot) {
    ...
    if (nextGameState.rows[move.to.row][move.to.column].type === "Crown")
      nextGameState.winner = bot;
  }
};
